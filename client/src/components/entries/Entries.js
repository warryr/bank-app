...
import { makeReport } from '../../apiRequests/entriesApiRequests';
import { yyyymmdd } from '../../util/convertationUtil';
...
const EntriesTableHead = () => (
  <thead>
  <tr>
    <th className='entries' rowSpan='2'>Счет, наименование</th>
    <th className='entries' colSpan='2'>Сальдо на начало периода</th>
    <th className='entries' colSpan='2'>Обороты за период</th>
    <th className='entries' colSpan='2'>Сальдо на конец периода</th>
  </tr>
  <tr>
...
class Entries extends React.Component {
  ...
  render() {
    ...
    return (
      <div className='view-info'>
        <button type='button' className='btn btn-light' onClick={this.onClose}>Закрыть день</button>
        <table className='entries' hidden>
          <EntriesTableHead/>
          <tbody>
          <tr>
            <td className='entries' colSpan='7'>BYN</td>
          </tr>
          ...
          <tr>
            <td className='entries' colSpan='7'>RUB</td>
          </tr>
          ...
          <tr>
            <td className='entries' colSpan='7'>USD</td>
          </tr>
          ...
          <tr>
            <td className='entries' colSpan='7'>EUR</td>
          </tr>
  ...
  onClose() {
    ...
    makeReport(
      entries => this.props.setEntries(entries),
...
const mapDispatchToProps = dispatch => ({
  setEntries: entries => dispatch({
    type: entriesActions.SET_ENTRIES,
    entries
  }),
});

export default LoggedOutRedirector(connect(mapStateToProps, mapDispatchToProps)(Entries));