...
import { NumberInput, Select, TextInput } from 'src/common/StatelessComponents'
...
import { addDeposit } from 'src/services/api/deposit'
import { yyyymmdd, sumDigits } from 'src/utils/convertationUtil'

Date.prototype.yyyymmdd = yyyymmdd

const depositTypes = [
  'Депозитный вклад до востребования',
  'Депозитный срочный вклад с выплатой процентов',
]
...
class DepositForm extends React.Component {
      ...
      <div className="view-info">
        <form>
          ...
          <Select
            ...
            options={depositCurrencies}
            onChange={this.updateIfCurrency}
          />
          <div className={this.state.depositHasTerm ? '' : 'hidden'}>
            <Select
              id="depositTerm"
              label="Срок депозита (месяцев)"
              options={this.state.depositTerms}
          ...
          <TextInput
            id="depositPercent"
            label="Процент по депозиту"
            value={this.state.depositPercent}
            readOnly
          />
          <TextInput
            id="depositStartDate"
            label="Дата начала депозитной программы"
            error={this.props.errors.depositStartDate}
          />
          <NumberInput
            id="depositAmount"
            label="Сумма депозита"
            error={this.props.errors.depositAmount}
          />
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (
      prevState.selectedTerm !== this.state.selectedTerm ||
  ...
  updateIfHasTerm() {
    const selectedType = $('#depositType')
  ...
  onAdd() {
    ...
    const dateParts = $('#depositStartDate')
      .val()
      .split('-')
    ...
    if (this.state.selectedTerm !== '') {
      endDate = new Date(
        startDate.setMonth(startDate.getMonth() + parseInt(this.state.selectedTerm)),
      )
    ...
    if (valid) {
      $('form')
        .find('#depositAmount, #depositStartDate')
        .val('')
